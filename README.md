...
WMLang 会遍历传入的文件夹，以传入的文件夹的子文件夹作为语言标签，子文件夹内所有 fvv 文件作为语言数据
...
```kotlin
object WMLang {
    private val _Hello by lazy { WMLangBase("你好", mapOf("en" to "Hello", "zh" to "你好")) }
    /** 你好
     * @suppress compose
     **/
    ...
    object Home {
        private val _Welcome by lazy { WMLangBase("欢迎%s", mapOf("zh" to "欢迎%s")) }
        /** 欢迎%s
         * @suppress compose
         * NA: en
         **/
...
由于要兼顾常规方式与 Jetpack Compose 方式两个场景，WMLang 以`getter`调用为常规方式，函数调用为 Jetpack Compose 方式，这可能并不是一个太好的解决方案，请在使用时留意调用的到底是哪一个
